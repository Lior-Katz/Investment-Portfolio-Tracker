...
public class SqlCommandBuilder
{
    ...
    {
        if (_connection != null) throw new MultipleSetQueryException("connection");
    ...
    public SqlCommandBuilder Table(string tableName)
    {
        ...
        if (_tableName != null) throw new MultipleSetQueryException("table name");
    ...
    {
        if (_command != null) throw new MultipleSetQueryException("command");
    ...
    public SelectQueryBuilder Select(List<string> columns)
    {
        ...
        if (_tableName != null) builder.From(_tableName);

        if (_searchPredicate != null) builder.Where(_searchPredicate);

        if (_columns != null) builder.Columns(_columns);

        if (_connection != null) builder.Connection(_connection);
        return builder;
    }
    ...
    public InsertQueryBuilder Insert(Dictionary<string, object> @params)
    {
        ...
        if (_tableName != null) builder.Into(_tableName);

        if (_params != null) builder.Params(_params);

        if (_outputColumns != null) builder.Output(_outputColumns);

        if (_connection != null) builder.SetConnection(_connection);
